...
import { getFolderSize, getAllFolders, exploreDirectory, watchDirectory, sendSavedDirectory, getMainDirectoryPath, getFileMetadata } from './lib/file-utils';
...
import { VectraDb } from './lib/vectra-db';
...
ipcMain.handle(IPC_CHANNELS.GET_DIRECTORY_STRUCTURE, async (event, dir: string) => {
...
ipcMain.handle(IPC_CHANNELS.MOVE_FILE, async (event, sourceFilePath: string, newPath: string) => {
  ...
  try {
    ...
    logger.info(`Successfully moved file from ${sourceFilePath} to ${newFilePath}`);
    return `Moved from ${sourceFilePath} to ${newFilePath}`;
  } catch (error) {
    logger.error(`Failed to move file from ${sourceFilePath} to ${newFilePath}:`, error);
...
ipcMain.handle(IPC_CHANNELS.PROCESS_DROPPED_ITEMS, async (event, items: string[]) => {
  const structure = await Promise.all(items.map((item) => exploreDirectory(item)));
  return structure;
});

// IPC handler for performing API requests
// TODO: refactor for search
ipcMain.handle(IPC_CHANNELS.PERFORM_API_REQUEST, async (event, data: { filePath: string }) => {
  ...
  // Call our tool
  const textData = await useDataExtractorTool(data.filePath, fileMeta.mimeType);

  // Call chain
  const sV = await searchVectorsChain(fileMeta, textData);

  // Send notification with info
  const notify = new Notification({'title': fileMeta.fileName + " indexed", body: textData})
  notify.show();
